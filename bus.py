...
maintitle=Label(top6,text="BUS BOOKING",font=("times new roman",35,"bold"),fg="white",bg="black")
...
list_source=["KALYAN","THANE","CST","PANVEL","PUNE"]
...
list_destination=["KALYAN","THANE","CST","PANVEL","PUNE"]
...
title11 = Label(travel_detail, text="Bus Type", font=("times new roman", 18) ,fg="black",bg="white")
...
title_bus = Label(travel_detail, text="Bus Time", font=("times new roman", 18) ,fg="black",bg="white")
title_bus.place(x=5, y=478)
list_source=["8:00 AM","8:30 AM","9:00 AM","9:30 AM","10:00 AM","10:30 AM","11:00 AM","11:30 AM","12:00 AM","12:30 PM","1:00 PM","2:00 PM","3:00 PM","4:00 PM","5:00 PM","6:00 PM","8:00 PM","9:00 PM","10:00 PM"]
...
def Receipt():
    ...
    txt_receipt.insert(END, 'Bus No:\t\t\t' + randomRef2 + "\n")
    txt_receipt.insert(END, 'Date:\t\t\t' + cal.get_date() + "\n")
    txt_receipt.insert(END, 'Bus Time:\t\t\t' + menu_head5.get() + "\n")
    ...
    txt_receipt.insert(END, 'Bus type:\t\t\t' + menu_head3.get() + "\n")
    if (ac_1.get() == 1):
    ...
    def submit1():
        if (otp_veri.get() == str(otp)):
            ...
            try:
                ...
                bustype=menu_head3.get()
                total=totalcost.get()
                # -------------------------------------------------------------Code for database-----------------------------------------------------------------------------------------------
                con = sqlite3.connect("user_registration")
                cur = con.cursor()
                cur.execute('Select * from user_bus_detail where email_id=?', (email,))
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "User already exist, please try with another Email", parent=top9)
                else:
                    cur.execute('insert into user_bus_detail(receipt_no,seat_no,bus_no,date,bus_time,first_name,last_name,gender,address,pin_code,mob_no,email_id,source,destination,bus_type,total_cost)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                        (receiptref, seatno, busno, date,bustime,first_name, last_name, genders, address, pincode, mobno, email, source, destination, bustype, total))
                    con.commit()
                    con.close()
