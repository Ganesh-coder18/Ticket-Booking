...
maintitle=Label(top6,text="TRAIN BOOKING",font=("times new roman",35,"bold"),fg="white",bg="black")
...
acnonac_detail= LabelFrame(customer_details_frame,bd=2,text="Ac/Non Ac Cost",font=("Times new roman", 15,"bold"),relief=RIDGE)
...
list_source=["KALYAN","CST","PANVEL","PUNE"]
...
list_destination=["SATARA","KOLHAPUR","AHMEDABAD","BANGALORE","PUNE","CHENNAI","DELHI"]
...
title11 = Label(travel_detail, text="Train Type", font=("times new roman", 18) ,fg="black",bg="white")
...
cal = Calendar(travel_detail,selectmode="day", day=15,moth=5,year=2021)
...
title_train = Label(travel_detail, text="Train Time", font=("times new roman", 18) ,fg="black",bg="white")
title_train.place(x=5, y=478)
list_source=["9:00 AM","12:00 PM","6:30 PM","11:30 PM"]
...
nonac_chk=Checkbutton(acnonac_detail,text="NON AC",variable=non_ac,onvalue=1,offvalue=0,command=NONAC,bg="white",font=("times new roman",18))
...
ac_chk=Checkbutton(acnonac_detail,text="AC",variable=ac_1,onvalue=1,command=AC,offvalue=0,bg="white",font=("times new roman",18))
...
def Total():
    if(menu_head1.get()=="KALYAN"):

        if(menu_head2.get()=="SATARA"):
            if(non_ac.get()==1):
                x=float(0)
                q1=float(190)
                q2=float(10)
        ...
        elif (menu_head2.get()=="CHENNAI"):
             messagebox.showerror("ERROR","NO TRAINS AVAILABLE")

        elif (menu_head2.get()=="AHMEDABAD"):
             messagebox.showerror("ERROR","NO TRAINS AVAILABLE")

        elif (menu_head2.get()=="BANGALORE"):
             messagebox.showerror("ERROR","NO TRAINS AVAILABLE")

        elif (menu_head2.get()=="DELHI"):
             messagebox.showerror("ERROR","NO TRAINS AVAILABLE")
...
def Receipt():
    ...
    txt_receipt.insert(END, 'PRN No:\t\t\t' + randomRef2 + "\n")
    txt_receipt.insert(END, 'Date:\t\t\t' + cal.get_date() + "\n")
    txt_receipt.insert(END, 'Train Time:\t\t\t' + menu_head5.get() + "\n")
    ...
    txt_receipt.insert(END, 'TRAIN type:\t\t\t' + menu_head3.get() + "\n")
    if (ac_1.get() == 1):
        txt_receipt.insert(END, 'AC cost:\t\t\t' + ac.get() + "\n")
    elif (non_ac.get()==1):
        txt_receipt.insert(END, 'NON-AC cost:\t\t\t' + nonac.get() + "\n")
...
def submit():
    ...
    def submit1():
        if (otp_veri.get() == str(otp)):
            ...
            try:
                receiptref=Receipt_Ref.get()
                seatno=randomRef1
                trainno=randomRef2
                date=cal.get_date()
                traintime=menu_head5.get()
                first_name =Firstname1.get()
                last_name =Lastname1.get()
                genders =menu_head.get()
                address =Address.get()
                pincode=pin_code.get()
                mobno=Mobile_no1.get()
                email=email_id1.get()
                source=menu_head1.get()
                destination=menu_head2.get()
                traintype=menu_head3.get()
                total=totalcost.get()
                # -------------------------------------------------------------Code for database-----------------------------------------------------------------------------------------------
                con = sqlite3.connect("user_registration")
                cur = con.cursor()
                cur.execute('Select * from user_train_detail where email_id=?', (email,))
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "User already exist, please try with another Email", parent=top9)
                else:
                    cur.execute('insert into user_train_detail(receipt_no,seat_no,train_no,date,train_time,first_name,last_name,gender,address,pin_code,mob_no,email_id,source,destination,train_type,total_cost)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                        (receiptref, seatno, trainno, date, traintime,first_name, last_name, genders, address, pincode, mobno, email, source, destination, traintype, total))
                    con.commit()
                    con.close()
                    messagebox.showinfo("success", "Register successfully", parent=top9)
                    try:
                        con = sqlite3.connect("user_registration")
                        cur = con.cursor()
                        cur.execute('Select * from user_train_detail where email_id=?', (email,))
